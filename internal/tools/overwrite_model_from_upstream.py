...
# while the migration is in flight. Each entry corresponds to a directory
# under upstream `model/` that we wholesale-replace into our `model/`. Keep
# in sync with `SC_UPSTREAM_MIGRATED_DIRS` in the Makefile.
NAMESPACES = ("gen-ai", "mcp", "openai")

# Group-level imports: upstream namespaces where we own only a subset of the
# groups inside a shared `model/<ns>/registry.yaml`. Each entry maps a local
# `model/<local_dir>/registry.yaml` target to the upstream file and the group
# id we extract from it. Keep in sync with `SC_UPSTREAM_MIGRATED_GROUPS` in
# the Makefile, which strips the same groups from the filtered upstream copy
# so Weaver does not see them defined twice.
GROUP_IMPORTS = (
    # local dir            upstream file               group id
    ("aws-bedrock",        "aws/registry.yaml",        "registry.aws.bedrock"),
)
...
def _extract_group(text: str, group_id: str) -> str:
    """Return the verbatim YAML block of the top-level group with the given id.

    Upstream registry files use 2-space indentation with each group introduced
    by `  - id: <id>`. We slice between that line and the next sibling group
    (or EOF) without parsing the YAML so comments and quoting survive intact.
    """
    lines = text.splitlines(keepends=True)
    start = next(
        (i for i, ln in enumerate(lines) if ln.startswith(f"  - id: {group_id}")),
        None,
    )
    if start is None:
        raise SystemExit(f"resync: group {group_id!r} not found in upstream file")
    end = next(
        (i for i in range(start + 1, len(lines)) if lines[i].startswith("  - id: ")),
        len(lines),
    )
    return "".join(lines[start:end])


def _import_group(clone_dir: Path, local_dir: str, upstream_rel: str, group_id: str) -> None:
    src = clone_dir / "model" / upstream_rel
    if not src.is_file():
        print(
            f"resync: upstream has no model/{upstream_rel} at this ref -- "
            f"namespace removal upstream? skipping {group_id}",
            file=sys.stderr,
        )
        return

    block = _extract_group(src.read_text(encoding="utf-8"), group_id)
    dst_dir = MODEL_DIR / local_dir
    dst_dir.mkdir(parents=True, exist_ok=True)
    dst = dst_dir / "registry.yaml"
    dst.write_text("groups:\n" + block, encoding="utf-8")
    print(f"imported {group_id} -> model/{local_dir}/registry.yaml", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="sc-upstream-") as tmp:
        ...
        for local_dir, upstream_rel, group_id in GROUP_IMPORTS:
            _import_group(clone_dir, local_dir, upstream_rel, group_id)
